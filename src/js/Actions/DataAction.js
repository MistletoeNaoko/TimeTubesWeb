    ...
    for (let i = 0; i < file.length; i++) {
        ...
        reader.onload = function () {
            ...
            if (i === file.length - 1) {
                ...
                let [spatialData, lookup] = extractData(headers, blazarData);
...
function extractData(spatialVal, data) {
    let result = [], lookup = {};
        ...
        if (polar && photo) {
            ...
            for (let key in dataHeaders['AUPolar']) {
                if (key == 'z') {
                    ...
                    if (!lookup[key])
                        lookup[key] = dataHeaders['AUPolar'][key];
                } else {
            ...
            for (let key in dataHeaders['AUPhoto']) {
                if (key == 'z') {
                    ...
                    if (!lookup[key])
                        lookup[key] = dataHeaders['AUPhoto'][key];
                } else {
                    if (key === 'V') {
                        // convert V to Flx(V)
